...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-6 flex items-center gap-4">
        <a href="{{ route('barang-keluar.index') }}" class="p-2 bg-white border border-gray-100 rounded-lg text-gray-400 hover:text-indigo-600 transition">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500">Stok barang akan otomatis berkurang setelah data disimpan.</p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        <form action="{{ route('barang-keluar.store') }}" method="POST" class="p-8 space-y-6" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="barang_id" id="barang_id" 
                            ...
                            required>
                        <option value="">-- Pilih Barang yang Keluar --</option>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_keluar" id="tanggal_keluar" 
                           ...
                           class="w-full px-4 py-3 rounded-xl border @error('tanggal_keluar') border-red-500 @else border-gray-200 @enderror focus:ring-2 focus:ring-indigo-500 outline-none transition"
                           required>
                    @error('tanggal_keluar') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                ...
                {{-- Keterangan --}}
                <div class="md:col-span-2">
                    <label for="keterangan" class="block text-sm font-semibold text-gray-700 mb-2">Alasan Keluar / Keterangan</label>
                    <textarea name="keterangan" id="keterangan" rows="2" 
                              ...
                              placeholder="Misal: Barang rusak, dikirim ke cabang, dll...">{{ old('keterangan') }}</textarea>
                </div>

                {{-- Foto Bukti --}}
                <div class="md:col-span-2 space-y-4">
                    <label class="block text-xs font-black text-slate-500 uppercase tracking-widest">
                        Bukti Pengeluaran (Foto Barang/Nota)
                    </label>
                    ...
                    <div class="flex items-center gap-6 p-4 bg-slate-50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <div class="flex-1">
                            <input type="file" name="foto" id="foto-input" accept="image/*"
                                   ...
                                   hover:file:bg-rose-700 transition-all cursor-pointer">
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-50">
                <a href="{{ route('barang-keluar.index') }}" class="px-6 py-2.5 text-sm font-medium text-gray-400 hover:text-gray-600 transition">Batal</a>
                <button type="submit" 
                        class="px-8 py-2.5 bg-rose-600 text-white rounded-xl hover:bg-rose-700 transition shadow-lg shadow-rose-100 font-bold">
                    Kurangi Stok
                </button>
            </div>
        </form>
