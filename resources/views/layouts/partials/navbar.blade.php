<nav class="bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-gray-100/80 transition-all duration-300" 
     x-data="{ open: false, isScrolled: false }" 
     @scroll.window="isScrolled = (window.pageYOffset > 20)"
     :class="isScrolled ? 'py-1 shadow-sm' : 'py-0'">
        ...
        <div class="flex justify-between h-20">
            ...
            <div class="flex items-center">
                <a href="{{ url('/') }}" class="group flex items-center gap-4 transition-all duration-500">
                    <div class="relative">
                        <img src="{{ asset('logo.png') }}" alt="Logo" class="w-11 h-11 rounded-xl shadow-lg group-hover:shadow-indigo-200 group-hover:rotate-6 transition-all duration-500">
                        <div class="absolute inset-0 rounded-xl bg-indigo-500/10 group-hover:bg-transparent transition-colors"></div>
                    </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:gap-6">
                ...
                @else
                    {{-- Notification --}}
                    <button class="relative w-10 h-10 flex items-center justify-center text-gray-400 hover:text-indigo-600 hover:bg-indigo-50/50 rounded-xl transition-all duration-300">
    ...
    <div x-show="open" 
        ...
        <div class="px-4 pt-2 pb-6 space-y-2">
            ...
            @else
                ...
                <a href="#" class="block px-4 py-3 text-sm font-bold text-gray-600 hover:bg-gray-50 rounded-xl transition-all">Dashboard</a>
                <a href="{{ route('logout') }}" 
