...
<div class="max-w-7xl mx-auto px-4 sm:px-6 pb-12 select-none space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        <div>
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Manajemen Barang Masuk</h1>
            <p class="text-xs font-medium text-slate-400 mt-1">Kelola dan pantau seluruh alur mutasi barang atau inventaris yang baru didistribusikan masuk.</p>
        </div>
    ...
    <div class="bg-slate-50/60 border border-slate-200/50 p-4 rounded-xl">
        <form action="{{ route('barang-masuk.index') }}" method="GET" class="flex flex-col sm:flex-row items-stretch sm:items-end gap-3">
            
            <div class="flex-1">
                <label rfor="merek" class="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5 ml-0.5">Filter Berdasarkan Merek</label>
                <div class="relative">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                @if(request('merek'))
                    <a href="{{ route('barang-masuk.index') }}" class="h-9 px-4 bg-slate-200/70 text-slate-600 rounded-lg text-xs font-medium hover:bg-slate-200 transition-colors flex items-center justify-center">
                        Reset
                    </a>
                @endif
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/70 border-b border-slate-100">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider w-28">Jumlah</th>
                        <th class="px-5 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Keterangan</th>
                        <th class="px-5 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider w-40">Tanggal Masuk</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/30 transition-colors">
                        ...
                        <td class="px-5 py-3.5 text-xs">
                            <span class="inline-flex items-center font-bold text-indigo-600 bg-indigo-50/50 px-2 py-0.5 rounded text-[11px] border border-indigo-100/30">
                                +{{ $b->jumlah }} Unit
                            </span>
                        </td>
                        ...
                        {{-- Tanggal Registrasi Masuk --}}
                        <td class="px-5 py-3.5 text-xs font-semibold text-slate-600">
                            {{ \Carbon\Carbon::parse($b->tanggal_masuk)->translatedFormat('d M Y') }}
                        </td>

                        {{-- Blok Manajemen Aksi --}}
                        <td class="px-5 py-3.5">
                            <div class="flex justify-center items-center gap-1">
                                <a href="{{ route('barang-masuk.edit', $b->id) }}" class="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-md transition-all" title="Ubah Riwayat">
                                ...
                                <form action="{{ route('barang-masuk.destroy', $b->id) }}" method="POST" onsubmit="return confirm('Hapus entri riwayat pasokan barang masuk ini?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="8" class="px-5 py-16 text-center">
                            <div class="flex flex-col items-center justify-center gap-2 text-slate-300">
                                ...
                                <p class="text-xs font-medium text-slate-400">Belum ada riwayat pencatatan log barang masuk dalam database gudang.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
